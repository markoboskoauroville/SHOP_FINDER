...
try:
    ...
    check("the banner says the version", c.wait_for("version 1", 8))                 # wait for the marker, then assert (four-tests.md §5)
    check("the key row is the one every app here has", c.wait_for("q quit   o open page   u check for update   r restart", 8), c.screen()[-200:])
    ...
    st, body = http("http://127.0.0.1:%d/" % c.port)
    check("the page has the gear and the settings panel in the first frame", b'id="gearBtn"' in body and b'id="settings"' in body and b'id="keyFile"' in body)
